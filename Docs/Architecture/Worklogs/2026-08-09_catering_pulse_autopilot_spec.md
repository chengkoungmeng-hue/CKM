...
graph TD
    ...
    B --> C[RSS Miner: devops/fetch_catering_pulse.py]
    ...
    B --> I[Indexing Pipeline: devops/notify_indexing.py]
...
| [fetch_catering_pulse.py](file:///c:/Projects/CKM/devops/fetch_catering_pulse.py) | RSS scraper, AI translator, and image compressor | Gemini fallback models list, 10s cooldown pacing, 429 backoff retry counts, word count constraints |
| [notify_indexing.py](file:///c:/Projects/CKM/devops/notify_indexing.py) | Sitemap index ping & IndexNow API submitter | Hostname, IndexNow Key (`c9b7e416a2d9426fa7406a09289196b0`), URL list scan path |
...
- **Feeds**: Go to [fetch_catering_pulse.py](file:///c:/Projects/CKM/devops/fetch_catering_pulse.py) and update the `FEEDS` array list.
...
- Go to [fetch_catering_pulse.py](file:///c:/Projects/CKM/devops/fetch_catering_pulse.py) line 361.
...
- **IndexNow Key**: If you rotate the IndexNow key, update `INDEXNOW_KEY` in [notify_indexing.py](file:///c:/Projects/CKM/devops/notify_indexing.py) and ensure the new token TXT file is served at root.
