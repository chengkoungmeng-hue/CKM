...
## 3.5 網址永久性與輸出驗證 (Identity & Validation — 必讀)

這一節記錄兩個已經上線過的嚴重錯誤，不要改回去。

### 網址一旦產生就永久固定

`id` 與 `slug` 曾經在每次執行時**依照清單位置重新編號**，而 slug 尾巴又帶著 id
（`{words}-pulse-NN`）。結果是每新增一篇文章，**全部 20 個網址同時改變**，前一天被
Google 和 Bing 收錄的網址全部 404，而 `notify_indexing.py` 又把新的一批送出去。

現行規則：

- `id` 由 `next_pulse_id()` 單調遞增產生，**永不重複使用、永不位移**。
- `slug` 由標題產生，**絕不編入位置編號**；碰撞時用 `source_link` 的短雜湊。
- 兩者在插入當下指派一次，之後**永遠不重算**。排序只影響顯示順序，不得移動任何網址。

### AI 輸出必須驗證後才寫入

Gemini 會在高棉文中夾帶泰文、中文或日文片段（實際發生過：`หัวใจ`、`วัฒนธรรม`、
`環境的控制與使用醬料調配提升風味`）。Hanuman 無法渲染這些字元，讀者看到的是豆腐方塊。

- 產生後先用 `find_foreign_scripts()` 檢查，**發現外文就整篇拒絕**，不要靜默剝除 ——
  從句子中間刪掉一個中文詞會留下語法破碎的高棉文。
- `scripts/check_content.py --strict` 在 CI 中於 commit 前再擋一次。
- 被拒絕不是失敗，明天會重新嘗試下一篇。寧可少一篇，不要上線亂碼。
